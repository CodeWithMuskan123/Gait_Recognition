...
def _pairwise_distances(embeddings, squared=False):
    ...
    # shape (batch_size,)
    square_norm = tf.linalg.diag(dot_product)
...
def _get_triplet_mask(labels):
    ...
    valid_indices = tf.math.logical_and(i_equal_j, tf.math.logical_not(i_equal_k))

    # Step 3 - combine two masks
    mask = tf.math.logical_and(distinct_indices, valid_indices)
